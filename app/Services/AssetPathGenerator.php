...
class AssetPathGenerator
{
    ...
    /**
     * Canonical storage key for Blender-converted GLB (not the native upload).
     */
    public function generatePreview3dConvertedGlbPath(Tenant $tenant, Asset $asset, int $version): string
    {
        if (! $tenant->uuid) {
            throw new \RuntimeException('Tenant UUID required for canonical storage path.');
        }
        ...
        return "tenants/{$tenant->uuid}/assets/{$asset->id}/v{$version}/previews/model_3d_converted.glb";
    }
}

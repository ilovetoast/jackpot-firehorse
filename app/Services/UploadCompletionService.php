...
class UploadCompletionService
{
    ...
    public function complete(
        ...
        if ($isBuilderStaged) {
            $brand = Brand::query()->find($targetBrandId);
            if ($brand) {
        ...
        return DB::transaction(function () use ($uploadSession, $fileInfo, $assetTypeEnum, $storagePath, $derivedTitle, $finalFilename, $filename, $metadataArray, $categoryId, $userId, $targetBrandId, $isBuilderStaged, $uploadOptions, $skipAiTagging, $skipAiMetadata, $isProstaffUpload) {
            $skipPublishForBuilderStaged = $isBuilderStaged;
            $approvalWorkflowNotified = false;
            $newAssetCreatedThisRequest = false;
            ...
            try {
                ...
                // Creator Phase 3: prostaff uploads start as pending (enforced again after category/plan logic).
                $initialApprovalStatus = $isProstaffUpload
                ...
                if ($isProstaffUpload && $userId !== null) {
                    $asset->submitted_by_prostaff = true;
                    $asset->prostaff_user_id = $userId;
                    $asset->save();
                }

                $newAssetCreatedThisRequest = true;
            ...
            // Creator Phase 3: prostaff — always pending + unpublished; overrides category / plan / contributor auto-publish.
            if (! $skipPublishForBuilderStaged && $isProstaffUpload && $userId !== null) {
                $approvalWorkflowNotified = $this->applyProstaffNewUploadApprovalEnforcement(
                    $asset,
                    $userId,
                    $approvalWorkflowNotified
                );
            }

            if ($newAssetCreatedThisRequest && $isProstaffUpload && $userId !== null && $targetBrandId !== null) {
                $brandForStats = Brand::query()->find($targetBrandId);
                $userForStats = User::query()->find($userId);
    ...
    protected function completeReplaceWithVersion(
        ...
        DB::transaction(function () use ($asset, $fileInfo, $uploadSession, $userId, $versionedPath, $newFilename, $comment) {
            ...
            // Phase 3: prostaff replace forces pending + unpublish (cannot bypass approval).
            $userForReplace = $userId ? User::find($userId) : null;
            $isProstaffReplace = $userForReplace && $asset->brand && $userForReplace->isProstaffForBrand($asset->brand);
            ...
            if ($requiresApproval || $isProstaffReplace) {
                $asset->approval_status = ApprovalStatus::PENDING;
                $asset->rejected_at = null;
                $asset->rejection_reason = null;
            }
            if ($isProstaffReplace && $userId !== null) {
                $asset->published_at = null;
                $asset->published_by_id = null;
                $asset->submitted_by_prostaff = true;
                if ($asset->prostaff_user_id === null) {
                    $asset->prostaff_user_id = $userId;
                }
            }
            // Non-prostaff replace: DO NOT touch published_at — existing enterprise DAM behavior.
    ...
    protected function completeReplaceInPlace(
        ...
        DB::transaction(function () use ($asset, $fileInfo, $uploadSession, $userId, $newFilename, $destPath, $comment) {
            $asset->size_bytes = $fileInfo['size_bytes'];
    ...
    protected function isProstaffUploader(?int $userId, ?int $brandId): bool
    {
        ...
        return $user !== null && $brand !== null && $user->isProstaffForBrand($brand);
    }

    /**
     * Force prostaff assets through approval: pending, unpublished, tagged; notify if not already sent this request.
     *
     * @return bool True if approvers were notified (either previously in this flow or now).
     */
    protected function applyProstaffNewUploadApprovalEnforcement(Asset $asset, int $userId, bool $alreadyNotified): bool
    {
        $asset->approval_status = ApprovalStatus::PENDING;
        $asset->published_at = null;
        $asset->published_by_id = null;

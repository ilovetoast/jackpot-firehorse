...
- **Flow** (only when a tenant is bound):
  ...
  7. Otherwise `AIService::executeAgent()` runs agent **`in_app_help_assistant`** (`config('ai.help_ask.agent_id')`) with task type **`in_app_help_action_answer`**, model **`gpt-4o-mini`** (registry key `gpt-4o-mini`), tenant + user + `brand_id` attribution. The prompt includes the top **`ai.help_ask.max_actions_for_prompt`** serialized **visible** actions as JSON (**`HELP_ACTIONS`**) **and** a second JSON object **`WORKSPACE_FACTS`** built from `PlanService` (plan name, `limits.*`, effective AI credits, storage usage, and any per-file-type registry caps tighter than the plan upload ceiling). The model must use **`WORKSPACE_FACTS`** for all numeric limit / quota questions and **`HELP_ACTIONS`** for routes and steps.
...
Guardrails are enforced in `HelpAiAskService` (prompt contract + `sanitizeAiPayload`). The model must not invent routes or numeric limits; `recommended_page.url` is re-bound from the retrieved payload when possible, and **upload/storage/credit numbers must come from `WORKSPACE_FACTS` in the prompt**, not from free-form product knowledge.

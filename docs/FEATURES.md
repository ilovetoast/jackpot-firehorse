...
- **Visibility cap:** Each brand has a maximum number of **non-hidden** categories per **asset** and **deliverable** library (default 20 each, `config/categories.php`). Hidden folders do not count. **Reference** / style-reference material is outside that cap. The tenant catalog API exposes remaining slots so the UI can disable adding **visible** templates at cap; see [SYSTEM_METADATA.md](SYSTEM_METADATA.md).
- **Default fields per template:** Platform admins edit **field bundles** on **System categories** (stored in `system_category_field_defaults`); legacy config remains a fallback. Existing brand categories are not auto-updated when bundles change—tenants reset to system default when they want new defaults. Global per-template suppression still overrides bundles.
- **New platform select options:** For existing tenants, new system option values may stay hidden until they use **Show new platform values** in Metadata → By category (or equivalent API). New signups see current options without that backlog. Details: [SYSTEM_METADATA.md](SYSTEM_METADATA.md).

...
# PID file directory
PID_DIR="$PROJECT_ROOT/storage/framework"
mkdir -p "$PID_DIR"
...
# Log files
SCHEDULER_LOG="$PROJECT_ROOT/storage/logs/scheduler.log"
WORKER_LOG="$PROJECT_ROOT/storage/logs/worker.log"
NPM_LOG="$PROJECT_ROOT/storage/logs/npm.log"
STRIPE_LOG="$PROJECT_ROOT/storage/logs/stripe.log"
...
setup_commands() {
    ...
    if [ -f "/.dockerenv" ] || ([ -f "/proc/self/cgroup" ] && grep -q docker /proc/self/cgroup 2>/dev/null); then
    ...
    elif [ -f "$PROJECT_ROOT/vendor/bin/sail" ]; then
        # On host - use Sail wrapper (relative to project root)
        ARTISAN_CMD="$PROJECT_ROOT/vendor/bin/sail artisan"
        NPM_CMD="$PROJECT_ROOT/vendor/bin/sail npm"
        echo -e "${CYAN}Detected: Running on host, using Sail wrapper${NC}"
    elif command -v php >/dev/null 2>&1 && command -v artisan >/dev/null 2>&1; then
        # Fallback: assume we can run commands directly
        ARTISAN_CMD="php artisan"
        NPM_CMD="npm"
        echo -e "${YELLOW}Warning: Using direct commands (Sail not detected)${NC}"
    else
        ...
        echo -e "${YELLOW}Looking for Sail at: $PROJECT_ROOT/vendor/bin/sail${NC}"
        exit 1
    fi
}

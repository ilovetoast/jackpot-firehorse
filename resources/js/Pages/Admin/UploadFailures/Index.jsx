...
export default function UploadFailuresIndex({ auth, uploads, stats, filters }) {
    ...
    const openDrawer = (uploadId) => {
        setDrawerOpen(true)
        setDrawerData(null)
        setDrawerLoading(true)
        fetch(`/app/admin/upload-failures/${uploadId}`)
            .then(res => res.json())
            .then(data => {
                setDrawerData(data)
    ...
    const handleFilter = (key, value) => {
        router.get(route('admin.upload-failures.index'), {
            ...filters,
            [key]: value,
            page: 1,
        })
    ...
    const failureReasonValue = (r) => (typeof r === 'string' ? r : r?.value ?? '—')
    ...
    return (
        ...
        >
            <Head title="Upload Failures - Admin" />
                    ...
                    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 mb-6">
                        <div className="flex flex-wrap gap-4">
                            ...
                            >
                                ...
                                <option value="presign_failed">Presign failed</option>
                                <option value="multipart_init_failed">Multipart init failed</option>
                                <option value="transfer_failed">Transfer failed</option>
                                <option value="finalize_failed">Finalize failed</option>
                                <option value="metadata_failed">Metadata failed</option>
                                <option value="thumbnail_failed">Thumbnail failed</option>
                    ...
                    <div className="bg-white overflow-hidden shadow-sm sm:rounded-lg border border-gray-200">
                        {!uploads.data?.length ? (
                            <div className="p-12 text-center">
                                ...
                                <h3 className="mt-2 text-sm font-medium text-gray-900">No failed uploads</h3>
                                <p className="mt-1 text-sm text-gray-500">
                                    No upload failures recorded.
                                </p>
                            </div>
                        ) : (
                            <>
                                    ...
                                    <table className="min-w-full divide-y divide-gray-200">
                                        ...
                                        <tbody className="bg-white divide-y divide-gray-200">
                                            {uploads.data.map((u) => (
                                                ...
                                                >
                                                    <td className="px-4 py-3 text-sm font-mono text-gray-900">{u.id.slice(0, 8)}…</td>
                                                    <td className="px-4 py-3 text-sm text-gray-700">{u.tenant?.name ?? '—'}</td>
                                                    <td className="px-4 py-3 text-sm text-gray-700">{u.stage ?? '—'}</td>
                                                    <td className="px-4 py-3 text-sm text-gray-700">{failureReasonValue(u.failure_reason)}</td>
                                                    <td className="px-4 py-3 text-sm text-gray-700">{u.failure_count ?? 0}</td>
                                                    <td className="px-4 py-3">
                                                        {u.ai_severity ? (
                                                            <span className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${getSeverityBadgeColor(u.ai_severity)}`}>
                                                                {u.ai_severity}
                                                            </span>
                                                        ) : (
                                                    ...
                                                    <td className="px-4 py-3 text-sm">{u.escalated ? 'Yes' : 'No'}</td>
                                                    <td className="px-4 py-3 text-sm text-gray-600">{formatDate(u.last_failed_at)}</td>
                                                    <td className="px-4 py-3">
                                ...
                                {(uploads.prev_page_url || uploads.next_page_url) && (
                ...
                <div className="fixed inset-0 z-50 overflow-hidden">
                    ...
                    <div className="fixed inset-y-0 right-0 max-w-xl w-full bg-white shadow-xl flex flex-col">
                        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
                            <h3 className="text-lg font-semibold text-gray-900">Upload Failure Details</h3>
                            <button
                                onClick={closeDrawer}
                        ...
                        <div className="flex-1 overflow-y-auto p-6">
                            {drawerLoading ? (
                                ...
                                <div className="space-y-6">
                                    <div>
                                        <p className="text-xs font-medium text-gray-500 uppercase">Upload ID</p>
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <p className="text-sm text-gray-900">{drawerData.stage ?? '—'}</p>
                                        </div>
                                        <div>

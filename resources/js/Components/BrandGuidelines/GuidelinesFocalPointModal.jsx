 ...
 * @param {string} [props.brandId] — required when saveMode is "builder"
 * @param {string} props.assetId
 * @param {'builder'|'library'} [props.saveMode] — builder = guidelines-only metadata; library = DAM focal + lock
 * @param {(fp: { x: number, y: number }|null) => void} props.onSaved
 */
export default function GuidelinesFocalPointModal({
    open,
    onClose,
    imageUrl,
    initialFocal,
    ...
    const handleSave = useCallback(async () => {
        if (!point || !assetId) return
        if (saveMode === 'builder' && !brandId) return
        setSaving(true)
        setError(null)
        try {
            const url =
                saveMode === 'library'
                    ? route('assets.focal-point.update', { asset: assetId })
                    : route('brands.brand-dna.builder.asset-guidelines-focal-point', { brand: brandId, asset: assetId })
            const res = await axios.patch(url, { x: point.x, y: point.y })
    ...
    }, [point, assetId, brandId, onSaved, onClose, saveMode])
        ...
        try {
            const url =
                saveMode === 'library'
                    ? route('assets.focal-point.update', { asset: assetId })
                    : route('brands.brand-dna.builder.asset-guidelines-focal-point', { brand: brandId, asset: assetId })
            await axios.patch(url, { clear: true })
            onSaved?.(null)
            onClose()
        } catch (err) {
            ...
            <div className="fixed inset-0 flex items-center justify-center p-4">
                <DialogPanel className="w-full max-w-lg rounded-2xl bg-[#111] border border-white/10 shadow-2xl p-5 text-white">
                    <DialogTitle className="text-lg font-semibold text-white/95">
                        {saveMode === 'library' ? 'Focal point (photo)' : 'Focal point for guidelines'}
                    </DialogTitle>

 ...
 *
 * @param {'card'|'embedded'} [props.variant] — `embedded` nests inside the Library footprint card (no outer chrome).
 */
export default function StorageInsightPanel({
    storage_insight: si,
    formatStorage,
    variant = 'card',
}) {
    ...
    const body = (
        <>
            <h3 className={titleClass}>Storage breakdown</h3>
    ...
    if (variant === 'embedded') {
        return (
            <div className="border-t border-slate-100 bg-slate-50/50 px-4 py-4 sm:px-5 sm:py-5">{body}</div>
        )
    }

    return (
        <div className="rounded-xl border border-slate-200/90 bg-slate-50/50 p-4 text-sm text-slate-700 sm:p-5">{body}</div>
    )
}

...
export default function FileTypeIcon({ 
    ...
    const getIcon = () => {
        ...
        // Design files (PSD, AI, EPS, Sketch, InDesign)
        if (['psd', 'psb', 'ai', 'eps', 'sketch', 'indd', 'idml'].includes(ext) || 
            mime.includes('photoshop') || mime.includes('illustrator') || mime.includes('indesign')) {
            return (
                <svg className={`${sizeClass} ${iconClassName}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        if (
            ...
            return <DocumentTextIcon className={`${sizeClass} ${iconClassName}`} aria-hidden />
        }
